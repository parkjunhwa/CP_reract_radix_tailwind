    ...
    <div className="space-y-3 pb-0">
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        ...
        <div className="panel p-5"><h3 className="t-text font-semibold text-sm mb-2">Donut Chart Widget</h3>
          <div className="flex items-center gap-3"><ResponsiveContainer width="45%" height={160}><PieChart><Pie data={pieData} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="v">{pieData.map((e,i)=><Cell key={i} fill={e.c}/>)}</Pie><Tooltip contentStyle={TS} formatter={(v) => `${Number(v ?? 0)}%`}/></PieChart></ResponsiveContainer>
      ...
      <div className="panel p-5"><h3 className="t-text font-semibold text-sm mb-4">Radial Bar — Quarterly Goals</h3>
        <ResponsiveContainer width="100%" height={180}><RadialBarChart data={radialData} innerRadius={30} outerRadius={110} startAngle={180} endAngle={0}><RadialBar dataKey="value" cornerRadius={6}/><Tooltip contentStyle={TS} formatter={(v) => `${Number(v ?? 0)}%`}/></RadialBarChart></ResponsiveContainer>
      </div>
    </div>
  );
}
