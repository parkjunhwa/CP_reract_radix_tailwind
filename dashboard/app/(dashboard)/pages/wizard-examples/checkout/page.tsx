...
export default function CheckoutWizardPage() {
  ...
  if (done) return (
    <div className="max-w-lg mx-auto panel p-10 text-center space-y-4">
      ...
      <button onClick={() => { setStep(1); setDone(false); }} className="h-10 px-6 rounded-lg text-white text-sm font-medium mt-2" style={{ backgroundColor: "var(--t-accent)" }}>Start New Order</button>
    </div>
  );
  ...
  return (
    <div className="space-y-4 pb-4 max-w-3xl">
            ...
            return (
              <div key={s.id} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={cn("w-9 h-9 rounded-full flex items-center justify-center border-2 transition-colors",
                    s.id < step ? "bg-emerald-500 border-emerald-500" : s.id === step ? "border-[var(--t-accent)] bg-[var(--luxe-accent-2)]" : "border-[var(--t-border-2)]")}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {/* Step content */}
        <div className="xl:col-span-2 panel p-6 space-y-4">
          {step === 1 && (
            <>
          ...
          {step === 2 && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[["First Name","James"], ["Last Name","Worthington"], ["Address","1 Fifth Avenue, Suite 100"], ["City","New York"], ["Country","United States"], ["ZIP","10001"]].map(([label, ph]) => (
                  <div key={label} className="space-y-1.5">
                    <label className="t-text-40 text-xs">{label}</label>
                    <input placeholder={ph} className={inputCls} style={inputStyle} />
                  </div>
                ))}
              </div>
            </>
          ...
          {step === 3 && (
            <>
              ...
              <div className="space-y-3">
                {[["Wire Transfer", "Standard for high-value orders", true], ["Bank Transfer", "ACH/SWIFT international", false]].map(([label, desc, checked]) => (
                  <label key={label as string} className="flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-colors"
                    style={{ borderColor: checked ? "var(--t-accent)" : "var(--t-border-2)", backgroundColor: checked ? "var(--luxe-accent-2)" : "transparent" }}>
                    <input type="radio" name="payment" defaultChecked={checked as boolean} className="mt-0.5 accent-violet-500" />
                    <div>
                      <p className="t-text-70 text-xs font-semibold">{label}</p>
                      <p className="t-text-40 text-[10px] mt-0.5">{desc}</p>
                    </div>
                  </label>
                ))}
              </div>
              <div className="space-y-3 mt-4">
                <div className="space-y-1.5">
                  <label className="t-text-40 text-xs">Bank / Institution</label>
                  <input placeholder="JPMorgan Chase" className={inputCls} style={inputStyle} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5"><label className="t-text-40 text-xs">Account Number</label><input placeholder="••••••••7823" className={inputCls} style={inputStyle} /></div>
                  <div className="space-y-1.5"><label className="t-text-40 text-xs">Routing</label><input placeholder="021000021" className={inputCls} style={inputStyle} /></div>
                </div>
              </div>
            </>
          )}
          {step === 4 && (
            <>
              ...
              <div className="space-y-3 text-xs">
                {[["Delivery to", "James Worthington, 1 Fifth Ave, New York"], ["Payment", "Wire Transfer — JPMorgan"], ["Auth. time", "24–48 business hours"], ["Shipping", "Armored vehicle — insured"]].map(([k, v]) => (
                  <div key={k as string} className="flex justify-between py-2 border-b" style={{ borderColor: "var(--t-border)" }}>
          ...
          <div className="flex justify-between pt-2">
            <button disabled={step === 1} onClick={() => setStep(s => s - 1)}
              ...
              style={{ borderColor: "var(--t-border-2)" }}>Previous</button>
            {step < 4
              ? <button onClick={() => setStep(s => s + 1)} className="h-9 px-4 rounded-lg text-white text-xs font-medium flex items-center gap-1.5" style={{ backgroundColor: "var(--t-accent)" }}>
        ...
        <div className="panel p-5 space-y-3 h-fit">
          ...
          <div className="pt-2 space-y-1.5 border-t" style={{ borderColor: "var(--t-border)" }}>
            ...
            <div className="flex justify-between text-sm font-bold pt-1 border-t" style={{ borderColor: "var(--t-border)" }}>
