  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {metrics.map(m => (
          <div key={m.label} className="panel p-4 flex flex-col gap-3">
            <p className="t-text-40 text-[11px] uppercase tracking-wide">{m.label}</p>
            <p className="t-text font-bold text-2xl">{m.value}</p>
            <div className={cn("flex items-center gap-1 text-xs font-semibold", m.positive ? "text-emerald-400" : "text-red-400")}>
              {m.positive ? <TrendingUp className="w-3 h-3" aria-hidden="true" /> : <TrendingDown className="w-3 h-3" aria-hidden="true" />}
              {m.change > 0 ? "+" : ""}{m.change}%
              <span className="t-text-30 font-normal ml-1">{m.period}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={ins.title} className="panel p-5 flex flex-col gap-2">
            <p className="t-text-40 text-xs uppercase tracking-wide">{ins.title}</p>
            <p className={cn("font-bold text-lg", ins.color)}>{ins.value}</p>
            <p className="t-text-40 text-xs">{ins.detail}</p>
            <button className="flex items-center gap-1 text-xs text-violet-400 hover:text-violet-300 mt-1">
              View details <ArrowUpRight className="w-3 h-3" aria-hidden="true" />
            </button>
          </div>
        ))}
      </div>
